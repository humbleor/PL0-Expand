...
{
    // k,j用于for循环, numOfFormat记录格式化输出的个数, perSegIndex是各个分段字符的index
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    {
										if(sym == casesym)   /* 准备按照case语句处理 */
										{
											casej=0;
											getsymdo;
											gendo(inte,0,1,0);  /*开辟一个地址空间保存"表达式"计算的结果*/
											memcpy(nxtlev,fsys,sizeof(bool)*symnum);
									        ...
									        gendo(sto,0,table[ptx2].adr+1,0);  /*把"表达式"结果保存在新开辟的空间,即之前最后一个变量的下一个地址*/
											if(sym == ofsym)
											{
												getsymdo;
											}
											...
											while(sym != endsym) /*case以end作为结束符*/
											{
												...
												while(sym == number)  /*case值表是一些用逗号分开的常数*/
												{
													...
													gendo(lit,0,num,0);
													gendo(opr,0,9,0);   /*判断值表中的常数是否与之前"表达式"计算的结果相等*/
													cx5[casei++]=cx;  /* 记录需要回填的地址的序号*/
													gendo(jpc,0,0,0);  /*如果相等,则跳转到此值表后面的语句继续执行,如果不等则继续比较些值表后面的常数*/
													getsymdo;
													if(sym==comma)
														getsymdo;
												}
												...
												{
													cx1=cx;
													gendo(jmp,0,0,0);  /*如果此值表的值都不等于"表达式"的结果,则跳转到下一个case语句*/
													for(k=0;k<casei;k++)  
												...
												gendo(jmp,0,0,0);  /*执行此case语句后,需要跳转出case循环*/
